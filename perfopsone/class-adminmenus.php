...
if ( ! class_exists( 'PerfOpsOne\AdminMenus' ) ) {
	class AdminMenus {
		...
		public static function initialize() {
			foreach ( apply_filters( 'init_perfops_admin_menus', [] ) as $menu => $submenus ) {
				if ( ! in_array( 'perfopsone-' . $menu, self::$slugs, true ) ) {
					switch ( $menu ) {
						case 'analytics':
							add_menu_page( esc_html__( 'Dashboard', 'ip-locator' ), sprintf( esc_html__( '%s Analytics', 'ip-locator' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_analytics_page' ], 'dashicons-chart-bar', 210874 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Dashboard', 'ip-locator' ), __( 'Dashboard', 'ip-locator' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_analytics_page' ], 0 );
							break;
						case 'tools':
							add_menu_page( esc_html__( 'Available Tools', 'ip-locator' ), sprintf( esc_html__( '%s Tools', 'ip-locator' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_tools_page' ], 'dashicons-admin-tools', 210876 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Tools', 'ip-locator' ), __( 'Available Tools', 'ip-locator' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_tools_page' ], 0 );
							break;
						case 'insights':
							add_menu_page( esc_html__( 'Available Reports', 'ip-locator' ), sprintf( esc_html__( '%s Insights', 'ip-locator' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_insights_page' ], 'dashicons-lightbulb', 210875 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Reports', 'ip-locator' ), __( 'Available Reports', 'ip-locator' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_insights_page' ], 0 );
							break;
						case 'records':
							add_menu_page( esc_html__( 'Available Catalogues', 'ip-locator' ), sprintf( esc_html__( '%s Records', 'ip-locator' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_records_page' ], 'dashicons-book', 210873 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Catalogues', 'ip-locator' ), __( 'Available Catalogues', 'ip-locator' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_records_page' ], 0 );
							break;
						case 'settings':
							add_menu_page( esc_html__( 'Control Center', 'ip-locator' ), sprintf( esc_html__( '%s Settings', 'ip-locator' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_settings_page' ], 'dashicons-admin-settings', 210871 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Control Center', 'ip-locator' ), __( 'Control Center', 'ip-locator' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_settings_page' ], 0 );
							break;
						case 'consoles':
							add_menu_page( esc_html__( 'Available Consoles', 'ip-locator' ), sprintf( esc_html__( '%s Consoles', 'ip-locator' ), 'PerfOps' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_consoles_page' ], 'dashicons-embed-generic', 210872 );
							add_submenu_page( 'perfopsone-' . $menu, esc_html__( 'Available Consoles', 'ip-locator' ), __( 'Available Consoles', 'ip-locator' ), 'manage_options', 'perfopsone-' . $menu, [ self::class, 'get_consoles_page' ], 0 );
							break;
					}
		...
		private static function display_as_lines( $items ) {
			...
			foreach ( $items as $item ) {
				$links   = [];
				$links[] = '<a href="' . $item['url'] . '"/>' . __( 'settings', 'ip-locator' ) . '</a>';
				$links[] = '<a href="' . $item['url'] . '&tab=about"/>' . __( 'about', 'ip-locator' ) . '</a>';
				$links[] = '<a href="https://wordpress.org/support/plugin/' . $item['slug'] . '" target="_blank">' . __( 'support', 'ip-locator' ) . '</a>';
				$links[] = '<a href="https://github.com/Pierre-Lannoy/wp-' . $item['slug'] . '" target="_blank">' . __( 'contribution', 'ip-locator' ) . '</a>';
				$disp   .= '<div class="po-container">';
				$disp   .= ' <div class="po-actionable">';
