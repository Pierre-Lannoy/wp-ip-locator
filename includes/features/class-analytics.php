...
class Analytics {
	...
	private function query_pie( $type, $limit ) {
		...
		$result .= '<div class="iplocator-pie-graph-handler" id="iplocator-pie-' . $group . '"></div>';
		$result .= '</div>';
		...
		$result .= ' new Chartist.Pie("#iplocator-pie-' . $group . '", data' . $uuid . ', option' . $uuid . ');';
		$result .= '});';
		$result .= '</script>';
	...
	private function query_top( $type, $limit ) {
		...
		while ( $cpt < $limit && array_key_exists( $cpt, $data ) ) {
			...
			$result .= '<img style="width:16px;vertical-align:bottom;" src="' . Favicon::get_base64( $data[ $cpt ]['id'] ) . '" />&nbsp;&nbsp;<span class="iplocator-top-line-title-text"><a href="' . esc_url( $url ) . '">' . $data[ $cpt ][ $group ] . '</a></span>';
			$result .= '</div>';
	...
	private function query_list( $type ) {
		...
		$detail_name  = esc_html__( 'Details', 'ip-locator' );
		$calls_name   = esc_html__( 'Calls', 'ip-locator' );
		$data_name    = esc_html__( 'Data Volume', 'ip-locator' );
		$latency_name = esc_html__( 'Latency', 'ip-locator' );
		...
		foreach ( $data as $key => $row ) {
			...
			$authorities = sprintf( esc_html( _n( '%d subdomain', '%d subdomains', $row['cnt_authority'], 'ip-locator' ) ), $row['cnt_authority'] );
			$endpoints   = sprintf( esc_html( _n( '%d endpoint', '%d endpoints', $row['cnt_endpoint'], 'ip-locator' ) ), $row['cnt_endpoint'] );
			switch ( $type ) {
				case 'sites':
					...
					$name = '<img style="width:16px;vertical-align:bottom;" src="' . Favicon::get_base64( $site ) . '" />&nbsp;&nbsp;<span class="iplocator-table-text"><a href="' . esc_url( $url ) . '">' . $site . '</a></span>';
					break;
				case 'domains':
					$detail = $authorities . ' - ' . $endpoints;
					$name   = '<img style="width:16px;vertical-align:bottom;" src="' . Favicon::get_base64( $row['id'] ) . '" />&nbsp;&nbsp;<span class="iplocator-table-text"><a href="' . esc_url( $url ) . '">' . $name . '</a></span>';
					break;
				case 'authorities':
					$detail = $endpoints;
					$name   = '<img style="width:16px;vertical-align:bottom;" src="' . Favicon::get_base64( $row['id'] ) . '" />&nbsp;&nbsp;<span class="iplocator-table-text"><a href="' . esc_url( $url ) . '">' . $name . '</a></span>';
				...
				case 'codes':
					...
					$name  = '<span class="iplocator-http iplocator-http-' . $http . '">' . $name . '</span>&nbsp;&nbsp;<span class="iplocator-table-text">' . Http::$http_status_codes[ $code ] . '</span>';
					$group = 'code';
					break;
				case 'schemes':
					...
					$name  = '<img style="width:14px;vertical-align:text-top;" src="' . $icon . '" />&nbsp;&nbsp;<span class="iplocator-table-text">' . strtoupper( $name ) . '</span>';
					$group = 'scheme';
					break;
				case 'methods':
					$name  = '<img style="width:14px;vertical-align:text-bottom;" src="' . Feather\Icons::get_base64( 'code', 'none', '#73879C' ) . '" />&nbsp;&nbsp;<span class="iplocator-table-text">' . strtoupper( $name ) . '</span>';
					$group = 'verb';
					break;
				case 'countries':
					if ( $other ) {
						...
						$name = '<img style="width:16px;vertical-align:baseline;" src="' . Flagiconcss\Flags::get_base64( strtolower( $name ) ) . '" />&nbsp;&nbsp;<span class="iplocator-table-text" style="vertical-align: text-bottom;">' . $country_name . '</span>';
					}
					$group = 'country';
					break;
			}
			$calls = Conversion::number_shorten( $row['sum_hit'], 2, false, '&nbsp;' );
			$in    = '<img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'arrow-down-right', 'none', '#73879C' ) . '" /><span class="iplocator-table-text">' . Conversion::data_shorten( $row['sum_kb_in'] * 1024, 2, false, '&nbsp;' ) . '</span>';
			$out   = '<span class="iplocator-table-text">' . Conversion::data_shorten( $row['sum_kb_out'] * 1024, 2, false, '&nbsp;' ) . '</span><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'arrow-up-right', 'none', '#73879C' ) . '" />';
			$data  = $in . ' &nbsp;&nbsp; ' . $out;
			if ( 1 < $row['sum_hit'] ) {
				$min = Conversion::number_shorten( $row['min_latency'], 0 );
				if ( false !== strpos( $min, 'K' ) ) {
					$min = str_replace( 'K', esc_html_x( 's', 'Unit symbol - Stands for "second".', 'ip-locator' ), $min );
				} else {
					$min = $min . esc_html_x( 'ms', 'Unit symbol - Stands for "millisecond".', 'ip-locator' );
				}
				$max = Conversion::number_shorten( $row['max_latency'], 0 );
				if ( false !== strpos( $max, 'K' ) ) {
					$max = str_replace( 'K', esc_html_x( 's', 'Unit symbol - Stands for "second".', 'ip-locator' ), $max );
				} else {
					$max = $max . esc_html_x( 'ms', 'Unit symbol - Stands for "millisecond".', 'ip-locator' );
				}
				$latency = (int) $row['avg_latency'] . '&nbsp;' . esc_html_x( 'ms', 'Unit symbol - Stands for "millisecond".', 'ip-locator' ) . '&nbsp;<small>(' . $min . '→' . $max . ')</small>';
			} else {
				$latency = (int) $row['avg_latency'] . '&nbsp;' . esc_html_x( 'ms', 'Unit symbol - Stands for "millisecond".', 'ip-locator' );
			}
	...
	private function query_map() {
		...
		$result .= ' },';
		$result .= ' onRegionTipShow: function(e, el, code){if (mapdata' . $uuid . '[code]){el.html(el.html() + " (" + mapdata' . $uuid . '[code] + " ' . esc_html__( 'calls', 'ip-locator' ) . ')")};},';
		$result .= ' });';
	...
	private function query_chart() {
		...
		$json_call        = wp_json_encode(
			[
				'series' => [
					[
						'name' => esc_html__( 'Success', 'ip-locator' ),
						'data' => $series_success,
					],
					[
						'name' => esc_html__( 'Error', 'ip-locator' ),
						'data' => $series_error,
					],
					[
						'name' => esc_html__( 'Quota Error', 'ip-locator' ),
						'data' => $series_quota,
					],
				],
			]
		...
		$json_data      = wp_json_encode(
			[
				'series' => [
					[
						'name' => esc_html__( 'Incoming Data', 'ip-locator' ),
						'data' => $series_kbin,
					],
					[
						'name' => esc_html__( 'Outcoming Data', 'ip-locator' ),
						'data' => $series_kbout,
					],
				],
			]
		...
		$json_uptime     = wp_json_encode(
			[
				'series' => [
					[
						'name' => esc_html__( 'Perceived Uptime', 'ip-locator' ),
						'data' => $series_uptime,
					],
				],
			]
		...
		$result .= '});';
		$result .= '</script>';
		$result .= '<div class="iplocator-multichart-item" id="iplocator-chart-data">';
		$result .= '</div>';
		$result .= '<script>';
		$result .= 'jQuery(function ($) {';
		...
		$result .= ' };';
		$result .= ' new Chartist.Line("#iplocator-chart-data", data_data' . $uuid . ', data_option' . $uuid . ');';
		$result .= '});';
		$result .= '</script>';
		$result .= '<div class="iplocator-multichart-item" id="iplocator-chart-uptime">';
		$result .= '</div>';
		$result .= '<script>';
		$result .= 'jQuery(function ($) {';
		...
		$result .= ' };';
		$result .= ' new Chartist.Line("#iplocator-chart-uptime", uptime_data' . $uuid . ', uptime_option' . $uuid . ');';
		$result .= '});';
		$result .= '</script>';
		$result .= '</div>';
	...
	private function query_kpi( $queried ) {
		...
		if ( 'call' === $queried ) {
			...
			if ( is_array( $data ) && array_key_exists( 'avg_latency', $data ) && ! empty( $data['avg_latency'] ) ) {
				$result[ 'kpi-bottom-' . $queried ] = '<span class="iplocator-kpi-large-bottom-text">' . sprintf( esc_html__( 'avg latency: %s ms.', 'ip-locator' ), (int) $data['avg_latency'] ) . '</span>';
			}
		}
		if ( 'data' === $queried ) {
			$data         = Schema::get_std_kpi( $this->filter, ! $this->is_today );
			...
			$in                                 = '<img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'arrow-down-right', 'none', '#73879C' ) . '" /><span class="iplocator-kpi-large-bottom-text">' . Conversion::data_shorten( $current_in, 2, false, '&nbsp;' ) . '</span>';
			$out                                = '<span class="iplocator-kpi-large-bottom-text">' . Conversion::data_shorten( $current_out, 2, false, '&nbsp;' ) . '</span><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'arrow-up-right', 'none', '#73879C' ) . '" />';
		...
		if ( 'server' === $queried || 'quota' === $queried || 'pass' === $queried || 'uptime' === $queried ) {
			...
			switch ( $queried ) {
				case 'server':
					$result[ 'kpi-bottom-' . $queried ] = '<span class="iplocator-kpi-large-bottom-text">' . sprintf( esc_html__( '%s calls in error', 'ip-locator' ), Conversion::number_shorten( $data_value, 2, false, '&nbsp;' ) ) . '</span>';
					break;
				case 'quota':
					$result[ 'kpi-bottom-' . $queried ] = '<span class="iplocator-kpi-large-bottom-text">' . sprintf( esc_html__( '%s blocked calls', 'ip-locator' ), Conversion::number_shorten( $data_value, 2, false, '&nbsp;' ) ) . '</span>';
					break;
				case 'pass':
					$result[ 'kpi-bottom-' . $queried ] = '<span class="iplocator-kpi-large-bottom-text">' . sprintf( esc_html__( '%s successful calls', 'ip-locator' ), Conversion::number_shorten( $data_value, 2, false, '&nbsp;' ) ) . '</span>';
					break;
				case 'uptime':
					if ( 0.0 !== $base_value ) {
						$duration = implode( ', ', Date::get_age_array_from_seconds( $this->duration * DAY_IN_SECONDS * ( 1 - ( $data_value / $base_value ) ), true, true ) );
						if ( '' === $duration ) {
							$duration = esc_html__( 'no downtime', 'ip-locator' );
						} else {
							$duration = sprintf( esc_html__( 'down %s', 'ip-locator' ), $duration );
						}
						$result[ 'kpi-bottom-' . $queried ] = '<span class="iplocator-kpi-large-bottom-text">' . $duration . '</span>';
					}
					break;
			}
		}
	...
	public function get_title_selector() {
		switch ( $this->type ) {
			case 'domains':
				switch ( $this->extra ) {
					case 'codes':
						$title = esc_html__( 'HTTP Codes Details', 'ip-locator' );
						break;
					case 'schemes':
						$title = esc_html__( 'Protocols Details', 'ip-locator' );
						break;
					case 'methods':
						$title = esc_html__( 'Methods Details', 'ip-locator' );
						break;
					case 'countries':
						$title = esc_html__( 'Countries Details', 'ip-locator' );
						break;
					default:
						$title = esc_html__( 'Domains Details', 'ip-locator' );
				}
				break;
			case 'domain':
				$title = esc_html__( 'Domain Summary', 'ip-locator' );
				break;
			case 'authorities':
				switch ( $this->extra ) {
					case 'codes':
					...
					case 'countries':
						...
						break;
					default:
						$title = esc_html__( 'Domain Details', 'ip-locator' );
				}
				$breadcrumbs[] = [
					'title'    => esc_html__( 'Domain Summary', 'ip-locator' ),
					'subtitle' => sprintf( esc_html__( 'Return to %s', 'ip-locator' ), $this->domain ),
					'url'      => $this->get_url(
						[ 'extra' ],
						[
							'type'   => 'domain',
				...
				break;
			case 'authority':
				$title         = esc_html__( 'Subdomain Summary', 'ip-locator' );
				$breadcrumbs[] = [
					'title'    => esc_html__( 'Domain Summary', 'ip-locator' ),
					'subtitle' => sprintf( esc_html__( 'Return to %s', 'ip-locator' ), $this->domain ),
					'url'      => $this->get_url(
			...
			case 'endpoints':
				switch ( $this->extra ) {
					...
					case 'countries':
						...
						break;
					default:
						$title = esc_html__( 'Subdomain Details', 'ip-locator' );
				}
				$breadcrumbs[] = [
					'title'    => esc_html__( 'Subdomain Summary', 'ip-locator' ),
					'subtitle' => sprintf( esc_html__( 'Return to %s', 'ip-locator' ), $this->subdomain ),
					'url'      => $this->get_url(
						[ 'extra' ],
						[
							'type'   => 'authority',
				...
				break;
			case 'endpoint':
				$title         = esc_html__( 'Endpoint Summary', 'ip-locator' );
				$breadcrumbs[] = [
					'title'    => esc_html__( 'Subdomain Summary', 'ip-locator' ),
					'subtitle' => sprintf( esc_html__( 'Return to %s', 'ip-locator' ), $this->subdomain ),
					'url'      => $this->get_url(
				...
				break;
			case 'country':
				$title    = esc_html__( 'Country', 'ip-locator' );
				$subtitle = L10n::get_country_name( $this->id );
				break;

		}
		$breadcrumbs[] = [
			'title'    => esc_html__( 'Main Summary', 'ip-locator' ),
			'subtitle' => sprintf( esc_html__( 'Return to IP Locator main page.', 'ip-locator' ) ),
			'url'      => $this->get_url( [ 'domain', 'id', 'extra', 'type' ] ),
		];
		$result        = '<select name="sources" id="sources" class="iplocator-select sources" placeholder="' . $title . '" style="display:none;">';
	...
	public function get_title_bar() {
		...
		$result .= '<span class="iplocator-title">' . $title . '</span>';
		$result .= '<span class="iplocator-subtitle">' . $subtitle . '</span>';
		$result .= '<span class="iplocator-datepicker">' . $this->get_date_box() . '</span>';
		$result .= '<span class="iplocator-switch">' . $this->get_switch_box( 'inbound' ) . '</span>';
		$result .= '<span class="iplocator-switch">' . $this->get_switch_box( 'outbound' ) . '</span>';
		$result .= '</div>';
		return $result;
	}
	...
	public function get_kpi_bar() {
		...
		$result .= '<div class="iplocator-kpi-large">' . $this->get_large_kpi( 'call' ) . '</div>';
		$result .= '<div class="iplocator-kpi-large">' . $this->get_large_kpi( 'data' ) . '</div>';
		$result .= '<div class="iplocator-kpi-large">' . $this->get_large_kpi( 'server' ) . '</div>';
		$result .= '<div class="iplocator-kpi-large">' . $this->get_large_kpi( 'quota' ) . '</div>';
		$result .= '<div class="iplocator-kpi-large">' . $this->get_large_kpi( 'pass' ) . '</div>';
		$result .= '<div class="iplocator-kpi-large">' . $this->get_large_kpi( 'uptime' ) . '</div>';
		$result .= '</div>';
		$result .= '</div>';
		return $result;
	}
	...
	public function get_main_chart() {
		if ( 1 < $this->duration ) {
			$help_calls  = esc_html__( 'Responses types distribution.', 'ip-locator' );
			$help_data   = esc_html__( 'Data volume distribution.', 'ip-locator' );
			$help_uptime = esc_html__( 'Uptime variation.', 'ip-locator' );
			$detail      = '<span class="iplocator-chart-button not-ready left" id="iplocator-chart-button-calls" data-position="left" data-tooltip="' . $help_calls . '"><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'hash', 'none', '#73879C' ) . '" /></span>';
			$detail     .= '&nbsp;&nbsp;&nbsp;<span class="iplocator-chart-button not-ready left" id="iplocator-chart-button-data" data-position="left" data-tooltip="' . $help_data . '"><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'link-2', 'none', '#73879C' ) . '" /></span>&nbsp;&nbsp;&nbsp;';
			$detail     .= '<span class="iplocator-chart-button not-ready left" id="iplocator-chart-button-uptime" data-position="left" data-tooltip="' . $help_uptime . '"><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'activity', 'none', '#73879C' ) . '" /></span>';
	...
	public function get_sites_list() {
		$result  = '<div class="iplocator-box iplocator-box-full-line">';
		$result .= '<div class="iplocator-module-title-bar"><span class="iplocator-module-title">' . esc_html__( 'Sites Breakdown', 'ip-locator' ) . '</span></div>';
		$result .= '<div class="iplocator-module-content" id="iplocator-sites">' . $this->get_graph_placeholder( 200 ) . '</div>';
		$result .= '</div>';
		$result .= $this->get_refresh_script(
			[
				'query'   => 'sites',
	...
	public function get_domains_list() {
		$result  = '<div class="iplocator-box iplocator-box-full-line">';
		$result .= '<div class="iplocator-module-title-bar"><span class="iplocator-module-title">' . esc_html__( 'All Domains', 'ip-locator' ) . '</span></div>';
		$result .= '<div class="iplocator-module-content" id="iplocator-domains">' . $this->get_graph_placeholder( 200 ) . '</div>';
		$result .= '</div>';
		$result .= $this->get_refresh_script(
			[
				'query'   => 'domains',
	...
	public function get_authorities_list() {
		$result  = '<div class="iplocator-box iplocator-box-full-line">';
		$result .= '<div class="iplocator-module-title-bar"><span class="iplocator-module-title">' . esc_html__( 'All Subdomains', 'ip-locator' ) . '</span></div>';
		$result .= '<div class="iplocator-module-content" id="iplocator-authorities">' . $this->get_graph_placeholder( 200 ) . '</div>';
		$result .= '</div>';
		$result .= $this->get_refresh_script(
			[
				'query'   => 'authorities',
	...
	public function get_endpoints_list() {
		$result  = '<div class="iplocator-box iplocator-box-full-line">';
		$result .= '<div class="iplocator-module-title-bar"><span class="iplocator-module-title">' . esc_html__( 'All Endpoints', 'ip-locator' ) . '</span></div>';
		$result .= '<div class="iplocator-module-content" id="iplocator-endpoints">' . $this->get_graph_placeholder( 200 ) . '</div>';
		$result .= '</div>';
		$result .= $this->get_refresh_script(
			[
				'query'   => 'endpoints',
	...
	public function get_extra_list() {
		switch ( $this->extra ) {
			case 'codes':
				$title = esc_html__( 'All HTTP Codes', 'ip-locator' );
				break;
			case 'schemes':
				$title = esc_html__( 'All Protocols', 'ip-locator' );
				break;
			case 'methods':
				$title = esc_html__( 'All Methods', 'ip-locator' );
				break;
			case 'countries':
				$title = esc_html__( 'All Countries', 'ip-locator' );
				break;
			default:
				$title = esc_html__( 'All Endpoints', 'ip-locator' );
		}
		$result  = '<div class="iplocator-box iplocator-box-full-line">';
		$result .= '<div class="iplocator-module-title-bar"><span class="iplocator-module-title">' . $title . '</span></div>';
		$result .= '<div class="iplocator-module-content" id="iplocator-' . $this->extra . '">' . $this->get_graph_placeholder( 200 ) . '</div>';
		$result .= '</div>';
		$result .= $this->get_refresh_script(
			[
				'query'   => $this->extra,
	...
	public function get_top_domain_box() {
		...
		$help    = esc_html__( 'View the details of all domains.', 'ip-locator' );
		$result  = '<div class="iplocator-40-module">';
		$result .= '<div class="iplocator-module-title-bar"><span class="iplocator-module-title">' . esc_html__( 'Top Domains', 'ip-locator' ) . '</span><span class="iplocator-module-more left" data-position="left" data-tooltip="' . $help . '">' . $detail . '</span></div>';
		$result .= '<div class="iplocator-module-content" id="iplocator-top-domains">' . $this->get_graph_placeholder( 200 ) . '</div>';
		$result .= '</div>';
		$result .= $this->get_refresh_script(
			[
				'query'   => 'top-domains',
	...
	public function get_top_authority_box() {
		...
		$detail  = '<a href="' . esc_url( $url ) . '"><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'zoom-in', 'none', '#73879C' ) . '" /></a>';
		$help    = esc_html__( 'View the details of all subdomains.', 'ip-locator' );
		$result  = '<div class="iplocator-40-module">';
		$result .= '<div class="iplocator-module-title-bar"><span class="iplocator-module-title">' . esc_html__( 'Top Subdomains', 'ip-locator' ) . '</span><span class="iplocator-module-more left" data-position="left" data-tooltip="' . $help . '">' . $detail . '</span></div>';
		$result .= '<div class="iplocator-module-content" id="iplocator-top-authorities">' . $this->get_graph_placeholder( 200 ) . '</div>';
		$result .= '</div>';
		$result .= $this->get_refresh_script(
			[
	...
	public function get_top_endpoint_box() {
		...
		$detail  = '<a href="' . esc_url( $url ) . '"><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'zoom-in', 'none', '#73879C' ) . '" /></a>';
		$help    = esc_html__( 'View the details of all endpoints.', 'ip-locator' );
		$result  = '<div class="iplocator-40-module">';
		$result .= '<div class="iplocator-module-title-bar"><span class="iplocator-module-title">' . esc_html__( 'Top Endpoints', 'ip-locator' ) . '</span><span class="iplocator-module-more left" data-position="left" data-tooltip="' . $help . '">' . $detail . '</span></div>';
		$result .= '<div class="iplocator-module-content" id="iplocator-top-endpoints">' . $this->get_graph_placeholder( 200 ) . '</div>';
		$result .= '</div>';
		$result .= $this->get_refresh_script(
			[
				'query'   => 'top-endpoints',
	...
	public function get_map_box() {
		...
		$detail  = '<a href="' . esc_url( $url ) . '"><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'zoom-in', 'none', '#73879C' ) . '" /></a>';
		$help    = esc_html__( 'View the details of all countries.', 'ip-locator' );
		$result  = '<div class="iplocator-60-module">';
		$result .= '<div class="iplocator-module-title-bar"><span class="iplocator-module-title">' . esc_html__( 'Countries', 'ip-locator' ) . '</span><span class="iplocator-module-more left" data-position="left" data-tooltip="' . $help . '">' . $detail . '</span></div>';
		$result .= '<div class="iplocator-module-content" id="iplocator-map">' . $this->get_graph_placeholder( 200 ) . '</div>';
		$result .= '</div>';
		$result .= $this->get_refresh_script(
			[
				'query'   => 'map',
	...
	public function get_codes_box() {
		...
		$detail  = '<a href="' . esc_url( $url ) . '"><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'zoom-in', 'none', '#73879C' ) . '" /></a>';
		$help    = esc_html__( 'View the details of all codes.', 'ip-locator' );
		$result  = '<div class="iplocator-33-module iplocator-33-left-module">';
		$result .= '<div class="iplocator-module-title-bar"><span class="iplocator-module-title">' . esc_html__( 'HTTP codes', 'ip-locator' ) . '</span><span class="iplocator-module-more left" data-position="left" data-tooltip="' . $help . '">' . $detail . '</span></div>';
		$result .= '<div class="iplocator-module-content" id="iplocator-code">' . $this->get_graph_placeholder( 90 ) . '</div>';
		$result .= '</div>';
		$result .= $this->get_refresh_script(
			[
				'query'   => 'code',
	...
	public function get_security_box() {
		...
		$detail  = '<a href="' . esc_url( $url ) . '"><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'zoom-in', 'none', '#73879C' ) . '" /></a>';
		$help    = esc_html__( 'View the details of protocols breakdown.', 'ip-locator' );
		$result  = '<div class="iplocator-33-module iplocator-33-center-module">';
		$result .= '<div class="iplocator-module-title-bar"><span class="iplocator-module-title">' . esc_html__( 'Protocols', 'ip-locator' ) . '</span><span class="iplocator-module-more left" data-position="left" data-tooltip="' . $help . '">' . $detail . '</span></div>';
		$result .= '<div class="iplocator-module-content" id="iplocator-security">' . $this->get_graph_placeholder( 90 ) . '</div>';
		$result .= '</div>';
		$result .= $this->get_refresh_script(
			[
				'query'   => 'security',
	...
	public function get_method_box() {
		...
		$detail  = '<a href="' . esc_url( $url ) . '"><img style="width:12px;vertical-align:baseline;" src="' . Feather\Icons::get_base64( 'zoom-in', 'none', '#73879C' ) . '" /></a>';
		$help    = esc_html__( 'View the details of all methods.', 'ip-locator' );
		$result  = '<div class="iplocator-33-module iplocator-33-right-module">';
		$result .= '<div class="iplocator-module-title-bar"><span class="iplocator-module-title">' . esc_html__( 'Methods', 'ip-locator' ) . '</span><span class="iplocator-module-more left" data-position="left" data-tooltip="' . $help . '">' . $detail . '</span></div>';
		$result .= '<div class="iplocator-module-content" id="iplocator-method">' . $this->get_graph_placeholder( 90 ) . '</div>';
		$result .= '</div>';
		$result .= $this->get_refresh_script(
			[
				'query'   => 'method',
	...
	private function get_large_kpi( $kpi ) {
		switch ( $kpi ) {
			case 'call':
				...
				$title = esc_html_x( 'Number of Calls', 'Noun - Number API calls.', 'ip-locator' );
				$help  = esc_html__( 'Number of API calls.', 'ip-locator' );
				break;
			case 'data':
				$icon  = Feather\Icons::get_base64( 'link-2', 'none', '#73879C' );
				$title = esc_html_x( 'Data Volume', 'Noun - Volume of transferred data.', 'ip-locator' );
				$help  = esc_html__( 'Volume of transferred data.', 'ip-locator' );
				break;
			case 'server':
				$icon  = Feather\Icons::get_base64( 'x-octagon', 'none', '#73879C' );
				$title = esc_html_x( 'Server Error Rate', 'Noun - Ratio of the number of HTTP errors to the total number of calls.', 'ip-locator' );
				$help  = esc_html__( 'Ratio of the number of HTTP errors to the total number of calls.', 'ip-locator' );
				break;
			case 'quota':
				$icon  = Feather\Icons::get_base64( 'shield-off', 'none', '#73879C' );
				$title = esc_html_x( 'Quotas Error Rate', 'Noun - Ratio of the quota enforcement number to the total number of calls.', 'ip-locator' );
				$help  = esc_html__( 'Ratio of the quota enforcement number to the total number of calls.', 'ip-locator' );
				break;
			case 'pass':
				$icon  = Feather\Icons::get_base64( 'check-circle', 'none', '#73879C' );
				$title = esc_html_x( 'Effective Pass Rate', 'Noun - Ratio of the number of HTTP success to the total number of calls.', 'ip-locator' );
				$help  = esc_html__( 'Ratio of the number of HTTP success to the total number of calls.', 'ip-locator' );
				break;
			case 'uptime':
				$icon  = Feather\Icons::get_base64( 'activity', 'none', '#73879C' );
				$title = esc_html_x( 'Perceived Uptime', 'Noun - Perceived uptime, from the viewpoint of the site.', 'ip-locator' );
				$help  = esc_html__( 'Perceived uptime, from the viewpoint of the site.', 'ip-locator' );
				break;
		}
	...
	private function get_switch_box( $bound ) {
		...
		$result = '<input type="checkbox" class="iplocator-input-' . $bound . '-switch"' . ( $checked ? ' checked' : '' ) . ' />';
		// phpcs:ignore
		$result .= '&nbsp;<span class="iplocator-text-' . $bound . '-switch"' . $opacity . '>' . esc_html__( $bound, 'ip-locator' ) . '</span>';
		$result .= '<script>';
		$result .= 'jQuery(function ($) {';
		$result .= ' var elem = document.querySelector(".iplocator-input-' . $bound . '-switch");';
