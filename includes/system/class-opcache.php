...
class OPcache {
	...
	public static function invalidate( $files, $force = false ) {
		...
		if ( function_exists( 'opcache_invalidate' ) && ! self::is_restricted() ) {
			...
			\DecaLog\Engine::eventsLogger( IPLOCATOR_SLUG )->info( sprintf( '%s: %d file(s).', $s, $cpt ) );
		}
		return $cpt;
	}
				...
				if ( 0 === strpos( $file, './' ) ) {
					foreach ( self::$do_not_compile as $item ) {
						if ( false !== strpos( $file, $item ) ) {
							\DecaLog\Engine::eventsLogger( IPLOCATOR_SLUG )->debug( sprintf( 'File "%s" must not be recompiled.', $file ) );
							continue 2;
						}
					}
					$file = str_replace( '..', '', $file );
					...
					if ( ! opcache_is_script_cached( $file ) ) {
						try {
							...
							if ( @opcache_compile_file( $file ) ) {
								...
								\DecaLog\Engine::eventsLogger( IPLOCATOR_SLUG )->debug( sprintf( 'Unable to compile file "%s".', $file ) );
							}
						} catch ( \Throwable $e ) {
							\DecaLog\Engine::eventsLogger( IPLOCATOR_SLUG )->debug( sprintf( 'Unable to compile file "%s": %s.', $file, $e->getMessage() ), [ 'code' => $e->getCode() ] );
						}
					} else {
						\DecaLog\Engine::eventsLogger( IPLOCATOR_SLUG )->debug( sprintf( 'File "%s" already cached.', $file ) );
					}
				}
			}
			\DecaLog\Engine::eventsLogger( IPLOCATOR_SLUG )->info( sprintf( 'Recompilation: %d file(s).', $cpt ) );
		}
		return $cpt;
	}
		...
		if ( $automatic && Option::network_get( 'warmup', false ) ) {
			...
			if ( function_exists( 'opcache_get_status' ) && ! self::is_restricted() ) {
				try {
					...
					\DecaLog\Engine::eventsLogger( IPLOCATOR_SLUG )->error( sprintf( 'Unable to query OPcache status: %s.', $e->getMessage() ), [ 'code' => $e->getCode() ] );
				}
			}
		}
	}
	...
	public static function warmup( $automatic = true, $force = false ) {
		...
		if ( Environment::is_wordpress_multisite() ) {
			\DecaLog\Engine::eventsLogger( IPLOCATOR_SLUG )->info( $automatic ? 'Network reset and warm-up initiated via cron.' : 'Network warm-up initiated via manual action.' );
		} else {
			\DecaLog\Engine::eventsLogger( IPLOCATOR_SLUG )->info( $automatic ? 'Site reset and warm-up initiated via cron.' : 'Site warm-up initiated via manual action.' );
		}
		...
		if ( Environment::is_wordpress_multisite() ) {
			\DecaLog\Engine::eventsLogger( IPLOCATOR_SLUG )->info( sprintf( 'Network warm-up terminated. %d files were recompiled', $result ) );
		} else {
			\DecaLog\Engine::eventsLogger( IPLOCATOR_SLUG )->info( sprintf( 'Site warm-up terminated. %d files were recompiled', $result ) );
		}
		return $result;
	}
