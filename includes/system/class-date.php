...
class Date {
	...
	public static function get_positive_time_diff_from_mysql_utc( $from ) {
		if ( strtotime( $from ) < time() ) {
			return sprintf( esc_html__( '%s ago', 'ip-locator' ), human_time_diff( strtotime( $from ) ) );
		} else {
			return esc_html__( 'currently', 'ip-locator' );
		}
	}
	...
	public static function get_time_diff_from_mysql_utc( $from ) {
		if ( strtotime( $from ) < time() ) {
			...
			return sprintf( esc_html__( 'in %s', 'ip-locator' ), human_time_diff( strtotime( $from ) ) );
		}
	}
	...
	public static function get_age_array_from_seconds( $age, $legend = false, $abbrev = false ) {
		if ( $abbrev ) {
			$intervals = [
				[ 60, _x( 'sec', 'Unit abbreviation - Stands for "second".', 'ip-locator' ), _x( 'sec', 'Unit abbreviation - Stands for "second".', 'ip-locator' ) ],
				[ 60, _x( 'min', 'Unit abbreviation - Stands for "minute".', 'ip-locator' ), _x( 'min', 'Unit abbreviation - Stands for "minute".', 'ip-locator' ) ],
				[ 100000, _x( 'hr', 'Unit abbreviation - Stands for "hour".', 'ip-locator' ), _x( 'hr', 'Unit abbreviation - Stands for "hour".', 'ip-locator' ) ],
			];
		} else {
			$intervals = [
				[ 60, _n( 'second', 'seconds', 1, 'ip-locator' ), _n( 'second', 'seconds', 2, 'ip-locator' ) ],
				[ 60, _n( 'minute', 'minutes', 1, 'ip-locator' ), _n( 'minute', 'minutes', 2, 'ip-locator' ) ],
				[ 100000, _n( 'hour', 'hours', 1, 'ip-locator' ), _n( 'hour', 'hours', 2, 'ip-locator' ) ],
			];
		}
		$value = [];
