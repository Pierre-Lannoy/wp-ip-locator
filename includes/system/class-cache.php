...
class Cache {
	...
	public static function init() {
		self::$ttls = [
			...
			'ip'                => 72 * HOUR_IN_SECONDS,
			'statistics'        => 12 * HOUR_IN_SECONDS,
		...
		if ( self::$apcu_available ) {
			if ( function_exists( 'apcu_cache_info' ) && function_exists( 'apcu_delete' ) ) {
				try {
					...
					\DecaLog\Engine::eventsLogger( IPLOCATOR_SLUG )->error( sprintf( 'Unable to query APCu status: %s.', $e->getMessage() ), [ 'code' => $e->getCode() ] );
					$result = 0;
				}
			}
		} elseif ( wp_using_ext_object_cache() ) {
	...
	public static function log_debug() {
		...
		if ( 0 !== (int) $analytics['hit']['count'] || 0 !== (int) $analytics['miss']['count'] ) {
			\DecaLog\Engine::eventsLogger( IPLOCATOR_SLUG )->debug( $log );
		}
	}
