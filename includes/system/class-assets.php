...
class Assets {
	...
	public function prefetch() {
		if ( Option::network_get( 'use_cdn' ) && IPLOCATOR_CDN_AVAILABLE ) {
	...
	public function register_style( $handle, $src, $file, $deps = [], $media = 'all' ) {
		if ( Option::network_get( 'use_cdn' ) && IPLOCATOR_CDN_AVAILABLE ) {
			if ( IPLOCATOR_ADMIN_URL === $src ) {
				$file = 'https://cdn.jsdelivr.net/wp/' . IPLOCATOR_SLUG . '/tags/' . IPLOCATOR_VERSION . '/admin/' . $file;
			} else {
				$file = 'https://cdn.jsdelivr.net/wp/' . IPLOCATOR_SLUG . '/tags/' . IPLOCATOR_VERSION . '/public/' . $file;
			}
			// phpcs:ignore
